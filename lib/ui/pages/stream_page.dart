                  ...
                  __.videoId != null
                      ? TextButton(
                          onPressed: () {
                            context
                                .read<StreamingBloc>()
                                .add(StreamingEvent.load(__.videoId!));
                          },
                          child: Row(
                            children: [Text(__.title!), Icon(Icons.skip_next)],
                          ))
                      : Container()
                ],
              ));
            ...
            controlsConfiguration: BetterPlayerControlsConfiguration(
                ...
                overflowMenuCustomItems: [
                  BetterPlayerOverflowMenuItem(
                      Icons.skip_next_outlined, 'Pular abertura (90s)', () {
                    skipOpen();
                  })
                ])),
...
BetterPlayerDataSource buildDataSourceNetwork(data, item) {
  if (item.location!.isNotEmpty && item.locationsd!.isNotEmpty) {
    return BetterPlayerDataSource(BetterPlayerDataSourceType.network,
        item.location == null ? item.locationsd! : item.location!,
        headers: DataHelpers.baseHeaders,
        resolutions: {"SD": item.location!, "HD": item.locationsd!},
...
BetterPlayerDataSource buildDataSourceLocal(data, item) {
  return BetterPlayerDataSource.file(
      '${GetIt.I<FlutterDownloaderDownloadController>().localPath}/${data[item.videoId]!.videoId}.mp4');
}
